...
function EntryCard({ entry }: { entry: WorkspaceEntry }) {
  return <Link href={entry.href} className="group flex min-h-48 flex-col border border-border bg-surface p-5 transition-all hover:-translate-y-0.5 hover:border-[var(--accent-border-hover)] focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-4 focus-visible:outline-[var(--focus-ring)] sm:p-6"><h2 className="text-2xl leading-tight text-ivory">{entry.title}</h2><p className="mt-3 text-sm leading-6 text-muted">{entry.description}</p><span className="mt-auto pt-6 text-sm font-medium text-accent-soft underline decoration-border underline-offset-4 group-hover:text-ivory">Bölümü aç →</span></Link>;
}
