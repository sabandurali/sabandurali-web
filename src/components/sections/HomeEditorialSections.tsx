...
const intelligenceItems = [
  ["Değerleme Analizleri", "/gayrimenkul-ve-istanbul/gayrimenkul-rehberleri"],
  ["Yatırım Analizleri", "/gayrimenkul-ve-istanbul/arastirmalar"],
  ["Mahalle Analizleri", districtGuidePath],
  ["İmar & Plan Bilgileri", "/gayrimenkul-ve-istanbul/sehir-ve-imar"],
  ["Piyasa Araştırmaları", "/arastirma-ve-analiz"],
] as const;
...
export function RealEstateIntelligence() {
  return <section className="bg-background"><div className="mx-auto max-w-[1440px] px-5 py-14 sm:px-8 sm:py-20 lg:px-10 lg:py-24"><SectionMark>02 / Gayrimenkul Intelligence</SectionMark><div className="mt-5 flex flex-col gap-5 border-b border-border pb-10 sm:flex-row sm:items-end sm:justify-between"><h2 className="max-w-3xl text-[2.7rem] font-semibold leading-[.96] text-ivory sm:text-6xl">Veriye dayalı kararlar.<br />Daha doğru gayrimenkul analizi.</h2><Link href="/gayrimenkul-ve-istanbul" className="min-h-11 text-sm font-semibold text-accent-soft underline decoration-border underline-offset-4 hover:text-ivory">Alanı keşfet →</Link></div><div className="mt-8 grid gap-px border border-border bg-border md:grid-cols-2 lg:grid-cols-5">{intelligenceItems.map(([title, href], index) => <Link key={title} href={href} className="group min-h-48 bg-surface p-5 transition-colors hover:bg-surface-soft sm:p-6"><span className="flex size-9 items-center justify-center border border-accent/50 text-accent-soft"><AreaIcon icon={index === 2 ? "city" : index === 4 ? "research" : "network"} className="size-4" /></span><h3 className="mt-8 font-serif text-2xl leading-tight text-ivory">{title}</h3><span className="mt-4 inline-block text-sm text-muted group-hover:text-accent-soft">İncele →</span></Link>)}</div></div></section>;
}

export function KnowledgeLibrary() {
  const cards = [["Araştırmalar & Analizler", "Veriyi, şehirleri ve güncel başlıkları kaynak temelli inceleyen yayın alanı.", "/arastirma-ve-analiz"], ["Kitaplar & Öğrenme", "Kitap incelemeleri, öğrenme pratikleri ve düşünme notları.", "/kitaplar-ve-ogrenme"], ["Yapay Zekâ & Teknoloji", "Dijital üretim ve teknolojinin pratik kullanımına yönelik yayınlar.", "/yapay-zeka-ve-teknoloji"]] as const;
  return <section className="bg-ivory-soft text-ink"><div className="mx-auto max-w-[1440px] px-5 py-14 sm:px-8 sm:py-20 lg:px-10 lg:py-24"><SectionMark>04 / Bilgi Kütüphanesi</SectionMark><h2 className="mt-5 text-[2.7rem] font-semibold leading-[.96] sm:text-6xl">Öğren.<br />Keşfet.<br />Geliştir.</h2><div className="mt-10 grid gap-5 lg:grid-cols-3">{cards.map(([title, description, href], index) => <Link key={title} href={href} className={`group flex min-h-72 flex-col border border-[var(--accent-border-soft)] p-6 transition-transform hover:-translate-y-0.5 sm:p-8 ${index === 1 ? "bg-background text-ivory" : "bg-ivory text-ink"}`}><p className={`text-[10px] font-semibold uppercase tracking-[.2em] ${index === 1 ? "text-accent-soft" : "text-accent-deep"}`}>0{index + 1}</p><h3 className="mt-auto font-serif text-3xl leading-tight">{title}</h3><p className={index === 1 ? "mt-4 text-sm leading-6 text-muted" : "mt-4 text-sm leading-6 text-muted-dark"}>{description}</p><span className={`mt-6 text-sm font-semibold ${index === 1 ? "text-accent-soft" : "text-accent-deep"}`}>Alanı aç →</span></Link>)}</div></div></section>;
}
